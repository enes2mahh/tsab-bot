...
export async function GET() {
  ...
  if (profile?.role !== 'admin') return NextResponse.json({ error: 'للأدمن فقط' }, { status: 403 })
  const results: Record<string, any> = {}

  // Test 1: simple email
  results.simple = await sendEmail({
    to: 'zero.anas123@gmail.com',
    subject: '✅ Sends Bot — اختبار Resend',
    html: `<div dir="rtl" style="font-family:Arial;padding:20px;background:#0F0F1A;color:#E2E8F0;border-radius:12px">
      ...
      <p>تم إرسال هذا الإيميل من Sends Bot عبر Resend API.</p>
