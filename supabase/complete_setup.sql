-- ============================================================
-- TSAB BOT - COMPLETE DATABASE SETUP SCRIPT
...
-- (uses ticket_messages as the replies table)
-- ============================================================
...
-- ============================================================
-- STEP 7: FILES, CONTACTS, TEMPLATES
-- ============================================================
...
CREATE POLICY "chatflow_sessions_own" ON chatflow_sessions
  FOR ALL USING (
    EXISTS (SELECT 1 FROM devices WHERE devices.id = chatflow_sessions.device_id AND devices.user_id = auth.uid())
    OR is_admin()
  );
...
-- PLANS (everyone can read active, admin manages all)
...
-- Auto-create trial subscription on first profile creation
...
BEGIN
  ...
  IF v_plan_id IS NOT NULL THEN
    INSERT INTO subscriptions (user_id, plan_id, status, messages_used, messages_limit, starts_at, expires_at)
    VALUES (NEW.id, v_plan_id, 'trial', 0, 1000, NOW(), NOW() + INTERVAL '3 days');
  END IF;
...
-- Resolve referral code from user metadata
...
BEGIN
  ...
  IF v_referral_code IS NOT NULL AND v_referral_code != '' THEN
    ...
    WHERE referral_code = upper(v_referral_code) AND id != NEW.id LIMIT 1;
...
-- Increment device message counter + subscription usage
...
BEGIN
  IF NEW.direction = 'outgoing' AND NEW.status = 'sent' THEN
    ...
    IF v_sub_id IS NOT NULL THEN
      UPDATE subscriptions SET messages_used = messages_used + 1 WHERE id = v_sub_id;
    END IF;
...
CREATE TRIGGER on_message_created
  AFTER INSERT ON messages FOR EACH ROW EXECUTE FUNCTION increment_device_messages();
...
VALUES
  ('Basic', 'الأساسية', 'basic', 'Perfect for starters', 'مثالية للبداية',
   39.00, 30, 3, 1000, 1,
   ...
   false, 1),
  ('Professional', 'الاحترافية', 'professional', 'For growing businesses', 'للأعمال المتنامية',
   79.00, 30, 7, 10000, 3,
   ...
   true, 2),
  ('Business', 'الأعمال', 'business', 'Full power for enterprises', 'القوة الكاملة للمؤسسات',
   99.00, 30, 0, 100000, 10,
...
-- ============================================================
-- STEP 13: STORAGE BUCKET (run if you want file uploads)
-- ============================================================

-- Uncomment and run these separately in Supabase SQL Editor:
/*
...
-- ============================================================
-- STEP 14: CREATE MASTER ADMIN ACCOUNT
-- ============================================================
-- IMPORTANT: First create the user in Supabase Auth Dashboard:
--   Authentication → Users → Add User
--   Email: admin@tsab-bot.com  (change as needed)
--   Password: YourSecurePassword123!  (change this!)
--   Then run the query below with the user's UUID:

-- REPLACE 'YOUR-USER-UUID-HERE' with the actual UUID from Supabase Auth
-- Example: UPDATE profiles SET role = 'admin', name = 'Super Admin' WHERE id = 'abc123...';

-- After creating the user in Auth dashboard, run:
/*
UPDATE profiles
SET
  role = 'admin',
  ...
  is_active = true,
  is_banned = false
WHERE email = 'admin@tsab-bot.com';  -- replace with your admin email
*/
...
-- Check plans:
SELECT name, name_ar, slug, price, is_active FROM plans;
